...
from src.gameplay.auto.visualizer import (
    AutoVisualAgentPlan,
    build_auto_visual_plans,
    normalize_adversarial_visual_algorithm,
)
...
class AutoModeMixin:
    ...
    def _start_simulation_mode(self) -> None:
        ...
        self.auto_visual_adversarial_algorithm = normalize_adversarial_visual_algorithm(
            getattr(self.settings, "selected_adversarial_algorithm", None)
        )
    ...
    def _init_auto_visual_demo(self) -> None:
        ...
        Nhóm 1-5 có 3 thuật toán; nhóm 6 là trận 1-vs-1 với Greedy.
        ...
        try:
            ...
            adversarial_algorithm = normalize_adversarial_visual_algorithm(
                getattr(self, "auto_visual_adversarial_algorithm", getattr(self.settings, "selected_adversarial_algorithm", None))
            )
            plans = build_auto_visual_plans(
                ...
                adversarial_algorithm=adversarial_algorithm,
            )
        ...
        if len(plans) == 2:
            ...
            # Dịch nhẹ 3 shipper/path để khi các thuật toán trùng đường vẫn nhìn thấy nhau.
            visual_offsets = [
                (-7, -7),
                (0, 0),
                (7, 7),
    ...
    def _handle_auto_visual_mouse_click(self, pos: tuple[int, int]) -> bool:
        """Handle Auto Visual controls that restart the current demo."""
        if int(getattr(self, "auto_visual_group_id", 1)) == 6:
            rects = getattr(self, "auto_visual_adversarial_button_rects", {})
            for algorithm, rect in rects.items():
                if rect and rect.collidepoint(pos):
                    algorithm = normalize_adversarial_visual_algorithm(algorithm)
                    self.auto_visual_adversarial_algorithm = algorithm
                    if hasattr(self.settings, "set_adversarial_algorithm"):
