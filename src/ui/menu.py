...
class MenuMixin:
    ...
    def _draw_algorithm_level_selector(self) -> None:
        ...
        level_x = 190
        level_y = 292
        self.adversarial_prev_button_rect = None
        self.adversarial_next_button_rect = None
        ...
        if group_id == 6:
            ...
            selected = str(getattr(self.settings, "selected_adversarial_algorithm", "ALPHA_BETA")).upper()
            if selected not in algorithms:
                selected = "ALPHA_BETA"

            algo_y = level_y + 132
            self._draw_text("AI", self.font_small, (255, 255, 255), level_x + 105, algo_y, center=True)
            self.adversarial_prev_button_rect = pygame.Rect(level_x, algo_y + 30, 46, 46)
            self.adversarial_next_button_rect = pygame.Rect(level_x + 184, algo_y + 30, 46, 46)
            algo_box = pygame.Rect(level_x + 52, algo_y + 24, 126, 58)

            self._draw_image_button(getattr(self, "ui_left_button", None), self.adversarial_prev_button_rect, "<", (65, 180, 85))
            self._draw_image_button(getattr(self, "ui_right_button", None), self.adversarial_next_button_rect, ">", (65, 180, 85))
            self._draw_panel(algo_box, alpha=185, border=True)
            self._draw_fitted_text(selected, self.font_tiny_bold, (255, 255, 255), algo_box.inflate(-8, -8), center=True)
    ...
    def _change_adversarial_algorithm(self, delta: int) -> None:
        algorithms = ("MINIMAX", "ALPHA_BETA", "EXPECTIMAX")
        selected = str(getattr(self.settings, "selected_adversarial_algorithm", "ALPHA_BETA")).upper()
        try:
            index = algorithms.index(selected)
        except ValueError:
            index = algorithms.index("ALPHA_BETA")

        algorithm = algorithms[(index + int(delta)) % len(algorithms)]
        self.settings.set_adversarial_algorithm(algorithm)
        self.auto_visual_adversarial_algorithm = algorithm
    ...
    def _handle_mouse_click(self, pos: tuple[int, int]) -> None:
        ...
        if hasattr(self, "adversarial_prev_button_rect") and self.adversarial_prev_button_rect and self.adversarial_prev_button_rect.collidepoint(pos):
            self._change_adversarial_algorithm(-1)
            return

        if hasattr(self, "adversarial_next_button_rect") and self.adversarial_next_button_rect and self.adversarial_next_button_rect.collidepoint(pos):
            self._change_adversarial_algorithm(1)
            return
