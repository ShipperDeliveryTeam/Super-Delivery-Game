...
def astar(start, goal, get_neighbors, heuristic) -> SearchResult:
    ...
    while queue:
        ...
        if current == goal:
            ...
            return SearchResult("ASTAR", path, cost[goal], expanded_nodes, generated_nodes, (perf_counter() - started_at) * 1000)
        ...
        for next_pos, step_cost in get_neighbors(current):
            ...
            heappush(queue, (f, order, next_pos))
            generated_nodes += 1

    return SearchResult("ASTAR", [], float("inf"), expanded_nodes, generated_nodes, (perf_counter() - started_at) * 1000)
