...
def get_valid_actions(
    ...
    # Có thể nhận thêm đơn nếu chưa đầy sức chứa.
    if len(partial.carrying) < capacity:
        for order_id in sorted(partial.waiting):
            actions.append(f"P_{order_id}")

    # Chỉ được giao các đơn đang mang.
    for order_id in sorted(partial.carrying):
...
def has_forward_solution(
    ...
    - Nếu chưa giao xong mà không còn hành động hợp lệ thì loại nhánh.
    """
...
def order_actions(
    ...
    """
    Cách sắp xếp action khác nhau tạo ra 3 kiểu CSP:

    - BACKTRACKING: xét theo thứ tự cơ bản.
    - FORWARD_CHECKING: ưu tiên action có cost gần nhất.
    - AC3_BACKTRACKING: ưu tiên delivery và cost thấp để giảm miền tìm kiếm.
    """
...
def run_ac3_precheck(order_ids: Sequence[str]) -> bool:
    ...
    - P_Oi phải tồn tại
    - D_Oi phải tồn tại
    - Ràng buộc precedence P_Oi trước D_Oi được giữ trong bước backtracking

    Trong mô hình này, AC-3 đóng vai trò lọc miền/ràng buộc trước khi tìm kiếm.
    """
